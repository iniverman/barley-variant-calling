...
rule mark_duplicates:
    ...
    shell:
        ...
        "/picard.jar MarkDuplicates -I {input} -O {output.marked_bam} -M {output.metrics}" 
...
rule haplotype_caller:
    ...
    shell:
        ...
        " | unzip gatk-4.5.0.0.zip | gatk-4.5.0.0/gatk --java-options  '-Xmx4g' " 
...
rule deepvariant:
    ...
    output:
        expand("calls/deepvariant/{vcf_deepvariant_name}.vcf.gz",vcf_deepvariant_name=config["vcf_deepvariant_name"])
    shell:
        " mkdir -p /data_deepvariant/ | mv  {input.ref_index} /data_deepvariant/"  
