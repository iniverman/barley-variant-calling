...
rule all:
    input: 
        ...
        expand("calls/bayes/{vcf_bayes_name}_sorted.vcf",vcf_bayes_name=config["vcf_bayes_name"]),
        ...
        expand("calls/deepvariant/{vcf_deepvariant_name}_snp.vcf",vcf_deepvariant_name=config["vcf_deepvariant_name"]),
        expand("calls/deepvariant/{vcf_deepvariant_name}_indel.vcf",vcf_deepvariant_name=config["vcf_deepvariant_name"])
...
rule index_samtools_genome:
    ...
    output:
        expand("data/{genome_name}.fai",genome_name=config["genome_name"])
    shell:
        " java -jar picard.jar CreateSequenceDictionary R={input} | samtools faidx {input}"
...
rule free_bayes:
    ...
    output:
        expand("calls/bayes/{vcf_bayes_name}.vcf",vcf_bayes_name=config["vcf_bayes_name"])
    shell:
        "freebayes -f {input.ref} {input.bam} --gvcf >{output}"
...
rule deepvariant:
    input:
        wd=expand("{wd}",wd=config["working_directory"]),
        ref_index=expand("data/{genome_name}.fai",genome_name=config["genome_name"]),
        ref=expand("data/{genome_name}",genome_name=config["genome_name"]),
        bam=expand("results/markdup/{sample}_nodup.bam",sample=config["samples_name"]),
    ...
    shell:
        "sudo docker run "
        "-v {input.wd}:/input "
        "-v {input.wd}/calls/deepvariant:/output "
        "google/deepvariant:1.6.0 "
        "/opt/deepvariant/bin/run_deepvariant  "
        "--model_type=WGS  "
        "--ref=/input/{input.ref} "
        "--reads=/input/{input.bam} "
        "--output_vcf=/output/call_deepvariant.vcf.gz  "
        "--output_gvcf=/output/call_deepvariant.g.vcf.gz  "
        "--num_shards=1"
...
rule sort_bayes: 
    input:
        bayes=expand("calls/bayes/{vcf_bayes_name}_filtered.vcf",vcf_bayes_name=config["vcf_bayes_name"]),
        
    output:
        s1=expand("calls/bayes/{vcf_bayes_name}_sorted.vcf",vcf_bayes_name=config["vcf_bayes_name"]),
        
    shell: 
        "bcftools sort {input.bayes} -o {output.s1}"

# Now we need to filter the bayes vcf beacuse it can be malformed, not having 
# a position in the reference genome, so we have to take out the rows that in the 
# reference genome has a "." indicating that the reference don't have a nucleotide
# recognized there. As input it will take the bayes_sorted vcf and as outp the bayes 
# filtered vcf. 
rule filter_bayes: 
    input: 
        bayes = expand("calls/bayes/{vcf_bayes_name}.vcf",vcf_bayes_name=config["vcf_bayes_name"])
    output:
        expand("calls/bayes/{vcf_bayes_name}_filtered.vcf",vcf_bayes_name=config["vcf_bayes_name"])
    shell: 
...
rule compress:
    input: 
        bayes=expand("calls/bayes/{vcf_bayes_name}_sorted.vcf",vcf_bayes_name=config["vcf_bayes_name"]),
        gatk=expand("calls/gatk/{vcf_gatk_name}.vcf",vcf_gatk_name=config["vcf_gatk_name"])
    output: 
        gz1=expand("calls/bayes/{vcf_bayes_name}_sorted.vcf.gz",vcf_bayes_name=config["vcf_bayes_name"]),
